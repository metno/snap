/*
  ...
  $Id: diContouring.cc,v 2.1 2006/05/31 10:26:02 lisbethb Exp $
	     ...
	     const Area& fieldArea)
{
  ...
  float zz[8], zzg[2][2], dclab[4], xlc[5], ylc[5], xlb[4], ylb[4];

  int   bit[nbitwd];

  int   err, i, j, k, k1, k2, l, n, n1, n2, n3, np1, np2, label;
  int   ix1, ix2, iy1, iy2, ix0, iy0, ixrange, iyrange;
  int   maxpos, mpos, liused, ibsize, jbsize, nxbox, nybox;
  int   iconv, nnfrmt, nnwdth, nzfrmt, nlabx, nlaby;
  int   ib, jb, istart, jstart, iend, jend, iud, nvalue;
  int   icolx, ltypx, iwidx, lev, iabeq;
  int   istartl, jstartl, ibox1, jbox1, kbox, iz1, iz2, jz1, jz2;
  int   ibdim, ij, ik, ibit, iwrd, ibit2, iwrd2,  newend;
  int   iend1, iend2, jend1, jend2, iside, iside1;
  int   npos, nposm1, linend, ic1, ic2, ic3, ic4, iab1, iab2;
  int   ns, np, nfrst, nlast, km, ni, nj, iz, jz, nzz, nspace2;
  int   nbx, nby, ibm, jbm;
  float chrx, chry, rchrx, rchry, undefv, zhigh;
  float xlim1, xlim2, ylim1, ylim2, dxeq, dyeq;
  float slab1f, dslab, stalab, endlab, dylab, dylabh, dxlab1, dxlab;
  float dxlab2, splim2, zmin, zmax, zminbl, zmaxbl, zmean;
  float zzstp, zzstp1, zzstp2, zc, ftmp, ftmp1, ftmp2, dxx, dyy;
  ...
  float xlab, ylab, xlabel, ylabel, xend, yend;
  float xbmap, ybmap, dxbx, dybx, dxby, dyby, xbm, ybm;
  float gx1, gx2, gy1, gy2;

  float *x, *y, *s, *xsmooth, *ysmooth;
  int   *iused, *indxgx, *indxgy, *kab;

  vector<ContourLine*> contourlines;
  bool highLeft;
  bool closed;
  ...
  if (nnfrmt != 0) {
    ...
    if (iconv==0) {
      ...
      nlabx = int(dylabh/sqrtf(cxy[1]*cxy[1]+cxy[4]*cxy[4])+0.5);
      nlaby = int(dylabh/sqrtf(cxy[2]*cxy[2]+cxy[5]*cxy[5])+0.5);
    }
  }
  ...
  while (j<=iy2 && !undefOriginal) {
    for (i=ix1; i<=ix2; i++)
      if (z[j*nx+i]==undefv) undefOriginal= true;
    j++;
  }
  ...
  if (undefOriginal) {
    for (j=iy1; j<iy2; j++) {
      for (i=ix1; i<ix2; i++) {
	int ij= j*nx+i;
	if (z[ij]   ==undefv || z[ij+1]   ==undefv ||
	    z[ij+nx]==undefv || z[ij+nx+1]==undefv) {
  ...
  if (undefOriginal) {
    ...
    for (i=0; i<nx*ny; ++i)
      z[i]= zOriginal[i];
    if (!drawBorders)
      replaceUndefinedValues(nx,ny,z,false);
  }
...
//###################################################################
  ...
  vector< map<float,int> > verAttach(ixrange+1);  // verAttach[i][y]
  vector< map<float,int> > horAttach(iyrange+1);  // horAttach[j][x]

  // borderConnect[ij][iside][line]
  map<int, map<int,int> > borderConnect;
  ...
  for (lev=firstLev; lev<nvalue; ++lev)
	...
	for (jb=0; jb<nybox; ++jb) {
	  for (ib=0; ib<nxbox; ++ib) {
	    zminb[jb][ib]= undefv*0.8;  // => unnecessary search !!!!!
	    zmaxb[jb][ib]= undefv;
	    iundef[jb][ib]= 0;
	  }
	}
		  ...
		  if (kab[ik]+kab[ik+1]==0) {
		    ...
		    if ((iused[iwrd] & bit[ibit]) == 0) {
			  ...
			  (((undefSquare[iwrd2] & bit[ibit2]) == 0) ||
			   zOriginal[j*nx+i]!=undefv ||
			   zOriginal[j*nx+i+1]!=undefv))
			goto startContour;
		    }
		  }
		}
	...
	i= int(x[1] + 0.5);
	j= int(y[1] + 0.5);
	prevundef= (zOriginal[j*nx+i]==undefv);
	...
	for (n=1; n<npos; n++) {
	  ...
	  if ((undefSquare[iwrd2] & bit[ibit2]) != 0) {
		...
		bool udef[4];
		udef[0]= (zOriginal[i+j*nx]      ==undefv);
		udef[1]= (zOriginal[i+1+j*nx]    ==undefv);
		udef[2]= (zOriginal[i+1+(j+1)*nx]==undefv);
		udef[3]= (zOriginal[i+(j+1)*nx]  ==undefv);
		...
		if (ds2 < ds1) {
	      ...
	      for (k=0; k<nuc-1; k++) {
	        ...
	        currentundef= (zOriginal[j*nx+i]==undefv);
	...
	for (n=1; n<npos; n++) {
	  ...
	  if (fabsf(dx)<0.1 || fabsf(dy)<0.1) {
	    ...
	    if ((z[j*nx+i]==undefv &&  highLeft) ||
		(z[j*nx+i]!=undefv && !highLeft)) {
	...
	int currcode,currend,currline,prevcode,prevend,prevline;
	int keepcode,keepend,keepline,keepconnect;
	float xydir;
	map<float,int>::iterator p,p1,p2;
      ...
      if (drawBorders && !closed && lev<nundef) {
		...
		vlp[ilp].yfirst= py;
		  ...
		  if ((undefSquare[iwrd2] & bit[ibit2]) == 0) {
		    ...
		    if (zOriginal[index]==undefv || zOriginal[index+nx]==undefv)
		      horAttach[j-iy0][px-0.01f]= line_code;
		    else
		      horAttach[j-iy0][px+0.01f]= line_code;
		  } else if (fabsf(py-y[n])<0.1) {
		    if (zOriginal[index]==undefv || zOriginal[index+1]==undefv)
	    ...
	    //...............................................................
	      ...
	      //...............................................................
	      if (undefOriginal && (zOriginal[index1]==undefv ||
	      			    zOriginal[index1+1]==undefv)) usepos=false;
	      //...............................................................
	      bool pdir;
		...
		pdir= false;
	      ...
	      //...............................................................
	      if (undefOriginal && (zOriginal[index1]==undefv ||
	      			    zOriginal[index1+nx]==undefv)) usepos=false;
	      //...............................................................
	      bool pdir;
	      if (!vlp[ilp].addfirst && !vlp[ilp].addlast)
		...
		if ((undefAllSquare[iwrd2] & bit[ibit2]) == 0) {
		    ...
		    if ((undefSquare[iwrd2] & bit[ibit2]) == 0) {
		      ...
		      if (zOriginal[index1]==undefv || zOriginal[index1+nx]==undefv)
		        horAttach[j-iy0][px1-0.01f]= line_code;
		      else
		        horAttach[j-iy0][px1+0.01f]= line_code;
		    } else if (fabsf(py1-y[n])<0.1) {
		      if (zOriginal[index1]==undefv || zOriginal[index1+1]==undefv)
		...
		if ((undefAllSquare[iwrd2] & bit[ibit2]) == 0) {
		    ...
		    if ((undefSquare[iwrd2] & bit[ibit2]) == 0) {
		      ...
		      if (zOriginal[index2]==undefv || zOriginal[index2+nx]==undefv)
		        horAttach[j-iy0][px2-0.01f]= line_code;
		      else
		        horAttach[j-iy0][px2+0.01f]= line_code;
		    } else if (fabsf(py2-y[n])<0.1) {
		      if (zOriginal[index2]==undefv || zOriginal[index2+1]==undefv)
	...
	for (n=2; n<=npos; ++n) {
	  dxx = x[n]-x[n-1];
	  dyy = y[n]-y[n-1];
	  s[n] = s[n-1] + sqrtf(dxx*dxx+dyy*dyy);
	}
	...
	if (drawBorders) {
	  ...
	  dxx = x[npos]-x[1];
	  dyy = y[npos]-y[1];
	  if (sqrtf(dxx*dxx+dyy*dyy)/(s[npos]-s[1])<0.90) dxylab=0.40;
	  else                                            dxylab=0.00;
	}
	...
	while (n3<npos) {
	  ...
	  while (label==0 && s[n+1]<smax && n<nposm1) {
		...
		if (iconv==2) {
		  ...
		  if (dxx != 0. || dyy != 0.)
		    nlabx = int(dylabh/sqrtf(dxx*dxx+dyy*dyy) + 0.5);
		  else nlabx=0;

		  dxx = (xz[ij+nx]-xz[ij]+xz[ij+1+nx]-xz[ij+1])*0.5;
		  dyy = (yz[ij+nx]-yz[ij]+yz[ij+1+nx]-yz[ij+1])*0.5;
		  if(dxx != 0. || dyy != 0.)
		    nlaby = int(dylabh/sqrt(dxx*dxx+dyy*dyy) + 0.5);
		  else nlaby = 0;
		}
  ...
  if (shading && drawBorders && borderConnect.size()) {
    ...
    map<int, map<int,int> >::iterator pbc=    borderConnect.begin();
    map<int, map<int,int> >::iterator pbcend= borderConnect.end();
    map<int,int>::iterator pc, pcend;
  ...
  if (shading && contourlines.size()>0) {
    ...
    map<float,int>::iterator p,pend;
    map<float,int>::reverse_iterator r,rend;
    vector<int> sideAttach;
    ...
    prevcode= sideAttach[nn-1];
    prevend=  prevcode/maxLines;
    prevline= prevcode%maxLines;
  ...
  if (shading && contourlines.size()>0) {
    ...
    fillContours(contourlines, nx, ny, z,
		 ...
		 psoutput, fieldArea,zrange,zstep,zoff);
  }
...
int sortlines(float zvmin, float zvmax, int mvalue, int nvalue,
	      ...
	      int nlines, float rlines[],
	      int ncol, int icol[], int ntyp, int ityp[],
	      int nwid, int iwid[], int nlim, float rlim[], float undefv)
{
		  ...
		  bool drawBorders, int iconv)
{
  ...
  ContourLine *cl;
  int n,nc,nstop,lr1,lr2,npos,i,ntot,nend,adduse[2];
  ...
  for (jc=0; jc<ncl; jc++) {
	...
	cl->undefRight= !cl->highLeft;
      ...
      if (ivalue<0 && (idraw==3 || idraw==4)) cl->outer= false;
