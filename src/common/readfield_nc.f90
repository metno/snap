...
! local variables
      ...
      if (istep < 0) then
c set all 'save' variables to default values,
c ncid not needed, will close automaticall
        ntav1 = 0
        ntav2 = 0
        return
      end if
...
c
      ...
      do while ((ntav2.eq.0) .and. (n.gt.0))
        if(iavail(n)%oHour.ge.ihdif1 .and.
     &     iavail(n)%oHour.le.ihdif2) then
          ntav2 = n
        end if
...
! open the correct file, if required
      if (file_name .ne. filef(iavail(ntav2)%fileNo)) then
        if (ncid.ne.0) then
...
!        if(ierror.ne.0) goto 100
c bug in chernobyl borders from destaggering
        do j=1,ny
          do i=1,nx
            if (v2(i,j,k) .ge. 1e+30) v2(i,j,k) = 0.
          end do
        end do
...
c..p0 for hybrid loaded to ptop, ap is a * p0
        if (ivcoor.ne.2 .and. .not.ptopv.eq.'') then
...
c  input ps, must be hPa, otherwise:
      if (nctype.eq.'arome'.or.nctype.eq.'dmi_eps'.or.
     &    nctype.eq.'ec_det'.or.nctype.eq.'h12_grib') then
        do j=1,ny
          do i=1,nx
...
c..the difference below may get negative due to different scaling
      ...
      else if (precstratiaccumv .ne. '') then
c accumulated stratiform and convective precipitation
c..precipitation between input time 't1' and 't2'
        if (timepos .ne. 1) then
          call calc_2d_start_length(start3d, count3d, nx, ny, -1,
     &            enspos, timepos - 1, has_dummy_dim)
...
c..the difference below may get negative due to different scaling
         unitScale = 1.
         if (nctype .eq. 'ec_det') unitScale = 1000.
          do j=1,ny
            do i=1,nx
...
c timepos eq 1, check if precipitation already present / assume dummy step 0
          call calc_2d_start_length(start3d, count3d, nx, ny, -1,
     &            enspos, timepos, has_dummy_dim)
          call nfcheckload(ncid, precstratiaccumv,
     +                     start3d, count3d, field3(1,1))
...
c..alevel and blevel are in the middle of each layer
        ...
        bhalf(1)=blevel(1)
        vhalf(1)=vlevel(1)
c..check if subselection of levels
        do k=2,nk-1
          if (klevel(k+1).ne.klevel(k)-1) then
...
c..size of each grid square (m**2)
        ...
        if (temp_is_abs) then
          rcp=r/cp
c create precomputed table for pressures between 0.1 and 1500hPa
          do i=1,15000
...
c t2thetafac is 50% faster, and less then 0.5% difference in theta
...
c..omega -> etadot, or rather etadot derived from continuity-equation (mean of both)
         call om2edot
      else if (sigmadotv.eq.'') then
c..omega -> etadot, or rather etadot derived from continuity-equation
        call om2edot
c om2edot take means of omega (=0) and continuity-equation, -> use only continuity equation
        do k=1,nk
          do j=1,ny
            do i=1,nx
              w2(i,j,k)=2.*w2(i,j,k)
