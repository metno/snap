...
sub runModel {
    ...
    my ($startTime, $startYY, $startmm, $startDD, $startHH);
    if ($params->{startTime} =~ /(\d{4})-(\d{2})-(\d{2})\s+(\d{1,2})/) {
        $startTime = "$1 $2 $3 $4";
        $startYY = sprintf "%02d", $1;
        $startmm = sprintf "%02d", $2;
        $startDD = sprintf "%02d", $3;
        $startHH = sprintf "%02d", $4;
    } else {
    ...
    if ($region) {
        ...
        system("bdiana -i snap.in -s /etc/diana/setup/diana.setup TIME='$timestring' p='$region' t=$startHH");
        system("rm -rf prod");
