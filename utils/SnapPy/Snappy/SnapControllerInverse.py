...
class SnapControllerInverse:
    ...
    def run_snap_query(self, qDict):
        ...
        for i in range(1, 100):
            ...
            if (
                startDT >= endDT
            ):  ### GEORGE: Prevents quick fix of swapping start and end times
...
"""
            ...
            if (
                metmodel == "nrpa_ec_0p1"
            ):  ##GEORGE: metmodel is defined above to be this - no other option pointless if statement)
                if "metpattern" in qDict:
                    files = self.res.getECMeteorologyFiles(
                        startDT, runTime, pattern=qDict["metpattern"]
                    )  ## GEORGE: Either change runTime to -runTime, or startDT to endDT
                    if len(files) == 0:
                        self.write_log(
