...
classnames= [(i, f"ASH_{i}") for i,_ in enumerate(diameters) if distrib[i] > 0]
...
def readEmepWriteSnap(infile, outfile, outsnap):
    ...
    with open(infile, 'rt') as ifh:
        for line in ifh:
            ...
            if row[FIELDS['BASE[km]']] == "VENT":
                row[FIELDS['BASE[km]']] = 0
...
if __name__ == "__main__":
    import os
    if os.path.exists("./columnsource_emission.csv"):
        readEmepWriteSnap("./columnsource_emission.csv",
                        "release.txt",
                        "snap.input")
    else:
